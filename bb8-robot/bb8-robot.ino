/**********************************************************************
  ...
  ----------
  (1)
  This software controls a drawing robot that comprises four
  omni-wheels arranged in a cross formation.

  (2)
  The motor controllers for each of the opposite wheels are connected
  to the SAME port(s) via 560 ohm isolating resistors such that they rotate
  in oppositie directions when looking towards the robot center (i.e the
  wheels move towards the target when viewed from above)

  (3)
  For all practical purposes this plotter behaves as a conventional
  XY plotter which means that its position may be controlled using
  Bresenham's line drawing formula.
...
// ----- plotter definitions
#define BAUD 9600
#define PAPER_WIDTH 1000              // Width in mm
#define PAPER_HEIGHT 1000             // Height in mm
#define PEN 3
...
#define IN1_X  A0                     // X-axis
#define IN2_X  A1
#define IN3_X  A2
#define IN4_X  A3

#define IN1_Y  8                      // Y-axis
#define IN2_Y  9
#define IN3_Y  10
#define IN4_Y  11
...
// ----- Motor definitions
/*
    The 28BYJ-48 5V DC motor has a "stride Angle" of 5.625/64 degrees per step.
    One complete revolution therefore requires 360/5.625*64 equals 4096 steps.
    The "half-stepping" motor pattern to achieve this is shown below.
*/
byte Motor[8] =
{ B00000001,
  B00000011,
...
byte Pattern = 0;                   // Motor[] array contents

#define STEPS_PER_MM 4096/(PI*58)   // 4096 steps/rev; 58mm diameter wheel
#define NUDGE STEPS_PER_MM*5        // Move pen 5mm (change number to suit)
unsigned long DELAY = 1;            // Delay (mS) between motor steps (controls speed)

int X_count = 0;                    // X step-counter
int X_count_max = STEPS_PER_MM * PAPER_WIDTH; // Boundary condition
byte X_pattern;                     // X Motor[] pattern

int Y_count = 0;                    // Y step-counter
int Y_count_max = STEPS_PER_MM * PAPER_HEIGHT; // Boundary condition
byte Y_pattern;                     // Y Motor[] pattern
...
void setup()
{
  ...
  digitalWrite(IN1_X, LOW);         // Stop motor
...
void left()
{
  ...
  if (X_count >= 0)
  {
    Index = X_count % 8;                  //calculate array index
    Pattern = PORTC;                      //get current motor pattern
    Pattern = Pattern & B11110000;        //preserve MSN
    Pattern = Pattern | Motor[Index];     //create new motor pattern
    PORTC = Pattern;                      //send new pattern to motor
...
void up()
{
  ...
  {
    Index = Y_count % 8;                 //calculate array index
...
void radials() 
{
  ...
  process(F("G00 X100 Y100"));

  // ----- draw octant 0 radials
  process(F("G01 X150 Y100"));
  process(F("G00 X100 Y100"));

  process(F("G01 X150 Y125"));
  process(F("G00 X100 Y100"));
  ...
  process(F("G01 X125 Y150"));
  process(F("G00 X100 Y100"));

  process(F("G01 X100 Y150"));
  process(F("G00 X100 Y100"));

  // ----- draw octant 2 radials
  process(F("G01 X75 Y150"));
  process(F("G00 X100 Y100"));
  ...
  process(F("G01 X50 Y125"));
  process(F("G00 X100 Y100"));

  process(F("G01 X50 Y100"));
  process(F("G00 X100 Y100"));

  // ----- draw octant 4 radials
  process(F("G01 X50 Y75"));
  process(F("G00 X100 Y100"));
  ...
  process(F("G01 X75 Y50"));
  process(F("G00 X100 Y100"));

  process(F("G01 X100 Y50"));
  process(F("G00 X100 Y100"));

  // ----- draw octant 6 radials
  process(F("G01 X125 Y50"));
  process(F("G00 X100 Y100"));
