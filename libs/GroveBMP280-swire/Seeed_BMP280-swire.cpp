...
bool BMP280::init(int sda,int scl)
{
  SWire.begin(sda,scl);
...
{
  SWire.beginTransmission(BMP280_ADDRESS);
  SWire.write(reg);
  SWire.endTransmission();

  SWire.requestFrom(BMP280_ADDRESS, 1);
...
uint16_t BMP280::bmp280Read16(uint8_t reg)
{
  ...
  SWire.requestFrom(BMP280_ADDRESS, 2);
...
uint32_t BMP280::bmp280Read24(uint8_t reg)
{
  ...
  SWire.requestFrom(BMP280_ADDRESS, 3);
  while(SWire.available()<3);
  data = SWire.read();
  data <<= 8;
  data |= SWire.read();
  data <<= 8;
...
{
  SWire.beginTransmission(BMP280_ADDRESS); // start transmission to device
  SWire.write(reg);       // send register address
