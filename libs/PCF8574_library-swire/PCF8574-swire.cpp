...
void PCF8574::readBuffer(bool force){
	if (millis() > PCF8574::lastReadMillis+READ_ELAPSED_TIME || _usingInterrupt || force){
		SWire.requestFrom(_address,(uint8_t)1);// Begin transmission to PCF8574 with the buttons
		lastReadMillis = millis();
		if(SWire.available())   // If bytes are available to be recieved
		{
			byte iInput = SWire.read();// Read a byte
...
void PCF8574::digitalWrite(uint8_t pin, uint8_t value){
	...
	SWire.beginTransmission(_address);     //Begin the transmission to PCF8574
	if (value==HIGH){
