...
// Legacy SWire.write() function fix
#if (ARDUINO >= 100)
  #define __wire_write(d) SWire.write(d)
#else
  #define __wire_write(d) SWire.send(d)
#endif


// Legacy SWire.read() function fix
#if (ARDUINO >= 100)
  #define __wire_read() SWire.read()
#else
  #define __wire_read() SWire.receive()
#endif
 ...
 */
BH1750::BH1750(byte addr,int iSDA,int iSCL) {
...
bool BH1750::configure(Mode mode) {
  ...
  switch (mode) {
    ...
    case BH1750::ONE_TIME_LOW_RES_MODE:
      ...
      // Send mode to sensor
      SWire.beginTransmission(BH1750_I2CADDR);
      __wire_write((uint8_t)BH1750_MODE);
      ack = SWire.endTransmission();
...
uint16_t BH1750::readLightLevel(bool maxWait) {
  ...
  // value
  SWire.requestFrom(BH1750_I2CADDR, 2);
