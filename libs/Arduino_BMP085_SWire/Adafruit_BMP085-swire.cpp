...
boolean Adafruit_BMP085::begin(uint8_t mode,int iSDA,int iSCL) {
...
uint8_t Adafruit_BMP085::read8(uint8_t a) {
  ...
  SWire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  SWire.write(a); // sends register address to read from
#else
  SWire.send(a); // sends register address to read from
#endif
  SWire.endTransmission(); // end transmission
  
  SWire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  SWire.requestFrom(BMP085_I2CADDR, 1);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = SWire.read(); // receive DATA
#else
  ret = SWire.receive(); // receive DATA
#endif
  SWire.endTransmission(); // end transmission
...
uint16_t Adafruit_BMP085::read16(uint8_t a) {
  ...
  SWire.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  SWire.requestFrom(BMP085_I2CADDR, 2);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = SWire.read(); // receive DATA
  ret <<= 8;
  ret |= SWire.read(); // receive DATA
...
#if (ARDUINO >= 100)
  SWire.write(a); // sends register address to read from
  SWire.write(d);  // write data
#else
  SWire.send(a); // sends register address to read from
  SWire.send(d);  // write data
#endif
  SWire.endTransmission(); // end transmission
}
